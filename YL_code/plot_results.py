...
def _plan_segments(solve) -> List[Tuple[str, float, float, str]]:
    ...
    for idx, trip in enumerate(solve.trips):
        ...
        if trip.charge_station_after_trip and trip.charge_time_after_trip > 0 and idx + 1 < len(solve.trips):
            # Charging block starts at trip_end (approximating the reposition inside the charge slab)
            start = trip.trip_end_time
            reposition_time = solve.trips[idx + 1].ready_time - start - trip.charge_time_after_trip
            segments.append(
                (
...
def plot_instance_map(
    instance,
    result,
    # title: str = "Spatial Layout: Stations, Requests, and Bus Routes",
    name: str = "Nodes distributions",
):
    """
    Plot charging stations, pickup/dropoff points, and bus routes.

    Parameters
    ----------
    instance : EaftInstance
        Instance generated by generate_toy_instance().
    result : SolveResult
        Output of solve_instance().
    save_path : Path | None
    ...
    # -----------------------------
    # 2. Plot request pickup/dropoff points
    # -----------------------------
    for request_id, request in instance.requests.items():
        ...
        ax.text(px, py + 0.4, f"{request_id}_p", fontsize=9)
        ...
        ax.text(dx + 0.12, dy + 0.12, f"{request_id}_d", fontsize=9)

        # optional: faint link between pickup and dropoff of same request
        ax.plot(
            [px, dx], [py, dy],
            linestyle=":",
            linewidth=1.0,
    ...
    # helper to convert a node_id to coordinates
def plot_instance_map(
    instance,
    result,
    name: str = "Nodes distributions",
    ...
    for idx, trip_report in enumerate(result.trips):
        ...
        # remove start-node circle
        # keep only end-node marker if desired
        ax.scatter(ex, ey, s=180, facecolors=color, edgecolors="black", linewidths=1.2, zorder=6)

        # optional: remove start text too
        # ax.text(sx + 0.25, sy - 0.25, f"start {bus_id}-T{trip_index}", color=color, fontsize=9)
        # ax.text(ex + 0.25, ey - 0.25, f"end {bus_id}-T{trip_index}", color=color, fontsize=9)

        # charging movement after trip
        if trip_report.charge_station_after_trip is not None:
            ...
            # connect charging station to the next trip's start node
            next_trip_report = next(
                (
                    t for t in result.trips
                    if t.bus_id == bus_id
                    and t.trip_index == trip_index + 1
                    and t.route
                ),
                None,
            )
            ...
            # use same solid arrow line style
            draw_arrow_link(ex, ey, cx, cy, color="black", lw=2.0, zorder=2)

            # ax.text(
            #     (ex + cx) / 2,
            #     (ey + cy) / 2 + 0.2,
            #     f"charge ({trip_report.charge_time_after_trip:.1f}, {trip_report.charge_energy_after_trip:.1f})",
            #     fontsize=8,
            #     color=color,
            # )

    # -----------------------------
    # 4. Initial station -> first trip
    # -----------------------------
...
def main() -> int:
    ...
    partial_instance, partial_result = _run_cross_evaluation("partial_recharge", requests=4)
    deep_instance, deep_result = _run_cross_evaluation("deep_recharge", requests=5)
    ...
    outputs = [
        ...
        plot_instance_map(
        partial_instance,
        partial_result.optimized_source,name="partial_instance_map"),
        plot_instance_map(
        deep_instance,
        deep_result.optimized_source,name="deep_instance_map")
        
    ]
    print("Generated figures:")
    print(partial_instance.requests)
    print(deep_instance.requests)
