...
class ChargingCurve:
    mode: str
    time_breakpoints: Tuple[float, ...]  # the ... means it can contain any number of float values
...
class EaftInstance:
    ...
    pickup_node: Dict[str, str]  # mapping from request_id to pickup node_id
    ...
    direct_service_time: Dict[str, float]   # direct travel time from pickup to dropoff
...
def _scale_charging_curve(   # The function takes an existing ChargingCurve and multiplies every value in time_breakpoints by time_scale, while leaving energy_breakpoints unchanged. So the total energy ladder stays the same, but the time needed to reach each energy level changes.
    curve: ChargingCurve,
    *,
...
def _scenario_overrides(
    ...
    if normalized == "partial_recharge":
        return (
            {
                3: (43.0, 54.0),
                4: (55.0, 66.0),   # # original: 3: (52.0, 107.0). 4: (72, 127)
            },
            {},
        )
...
def generate_toy_instance(
    ...
    *,
    ...
    station_templates = (   # station name, station coordinate, time scale for charging, unit charging cost
    ...
    requests: Dict[str, Request] = {}   # Dict[,]: specify types of keys and values
    nodes: Dict[str, NodeData] = {}
    pickup_node: Dict[str, str] = {}
    dropoff_node: Dict[str, str] = {}

    # print(stations, station_coords, requests)
    ...
    for request_idx in range(num_requests):
        ...
        earliest = 12.0 + 20.0 * request_idx  # pickup time window
        latest = earliest + 55.0    # pickup time window
    ...
    for station, coord, _, _ in station_templates[:num_stations]:   # ("depot", (0.0, 0.0), 1.08, 0.48)
    ...
    charging_curve = make_charging_curve(charging_mode)   # return ChargingCurve object
    station_curves = {
        station: _scale_charging_curve(charging_curve, time_scale=time_scale)  # change the charging speed
    ...
    time_horizon = latest_pickup + num_trip_slots * (max_station_charge_time + 3.0 * max_leg_time) + 30.0   # It is a rough upper bound on the total time window of the whole toy instance.
...
def solve_instance(
    ...
    *,
    ...
    if fixed_plan is not None:  # If a previously saved binary plan is provided, fix the model’s key binary variables to that plan before solving.
    ...
    for bus_id in buses:
        ...
        for previous_trip, next_trip in zip(trip_slots[:-1], trip_slots[1:]):
            ...
            for station in stations:
                ...
                model.addGenConstrPWL(  # add a piecewise-linear constraint
                    charge_time_station[bus_id, station, previous_trip],
                    charge_energy_station[bus_id, station, previous_trip],
                ...
                ) # if charge_station[bus_id, station, previous_trip] == 0 then charge_time_station[bus_id, station, previous_trip] == 0.
                model.addGenConstrIndicator(
                    charge_station[bus_id, station, previous_trip],
                    False,
            ...
            model.addConstr(  # defines the travel time from the chosen charging station to the first service node of the next trip
    ...
    # A.5: symmetry breaking for identical (homogeneous) buses. # if a later-numbered bus is used, then the earlier-numbered bus must also be used
    if len(buses) >= 2:   
...
def cross_evaluate_modes(
    ...
    *,
    ...
    fixed_plan_in_target = solve_instance(
        ...
        mip_gap=mip_gap,
        verbose=verbose,
        fixed_plan=optimized_source.fixed_plan,  # keep the same discrete plan from the source solve "same trip activation, same chosen charging station, same first/last nodes, same route arcs", then re-solve only the remaining continuous parts, like timing, battery, and charging amounts.
    )
