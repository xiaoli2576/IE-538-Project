...
\begin{block}{}
\emph{If the planner optimizes under a constant-rate assumption, will the resulting plan still be feasible once a real tapering battery has to execute it?}
\end{block}
...
\textbf{Headline setup.} 1 bus, 3 trip slots, 2 charging stations, 7 synthetic requests.\\
\textbf{Robustness setup.} 2 buses, 2 trip slots each, 8 requests (covered after the headline result).\\
\textbf{Parameters.} Battery 36 kWh, min 4 kWh, initial 22 kWh, capacity 3 pax. Penalty \$18/unserved.

\vspace{6pt}
\begin{block}{Three diagnostic scenarios (same 7-request instance, differ only in pickup window tightness)}
\begin{tabular}{@{}p{0.19\textwidth}p{0.75\textwidth}@{}}
\textbf{Baseline}       & all 7 windows loose ($\sim$55 min wide); both modes should serve all 7 (sanity check). \\
\textbf{Partial recharge} & \texttt{r3,r4} squeezed to $\sim$10-min windows; a moderate recharge between trips fits the squeezed pair $\Rightarrow$ stresses the fast early portion of the taper. \\
\textbf{Deep recharge}  & \texttt{r3,r4,r5} all squeezed back-to-back; the third trip needs a substantial inter-trip recharge $\Rightarrow$ stresses the slow tail of the taper.
\end{tabular}
\end{block}
...
\begin{itemize}\itemsep1pt
  ...
  \item \emph{Cross-evaluation}: optimize under linear, freeze the discrete plan (route, station, trip composition), re-solve under nonlinear, then re-optimize freely under nonlinear.
\end{itemize}
\end{frame}

% ================= RESULTS TABLE =================
\begin{frame}{Direct Comparison --- Headline Numbers ($|Q|=7$, $|R|=3$)}
\centering
\small
...
\small Both modes serve 7/7 at 353.64. Nonlinear charges \emph{longer} (41.27 vs 24.57 min) because the 35.95 kWh demand reaches the slow tail of the taper.
\end{block}
\column{0.32\textwidth}
\begin{block}{Partial recharge}
\small Both modes settle on the same 5-served plan and the same 25.10 kWh recharge. Charging realism only changes the \emph{time}, not the served set.
\end{block}
\column{0.32\textwidth}
\begin{block}{Deep recharge --- headline}
\small Linear serves 5; nonlinear serves \alert{\textbf{4}}. \alert{\textbf{Gap = 56.44}}. Linear commits to 35.81 kWh; nonlinear refuses and falls back to 23.00 kWh.
\end{block}
\end{columns}
\end{frame}

% ================= MECHANISM (the core slide) =================
\begin{frame}{Why the Two Models Disagree in Deep Recharge}
\textbf{Time windows:} \texttt{r3}$\in[50,60]$, \texttt{r4}$\in[61,71]$, \texttt{r5}$\in[72,82]$, \texttt{r6}$\in[83,93]$. Hub time scale 0.82, depot 1.08.
...
\begin{itemize}\itemsep1pt
  ...
  \item Trip 3 is therefore ready in time to catch the late \texttt{r7}. \textbf{Obj 165.51}.
\end{itemize}
\end{block}
...
\begin{itemize}\itemsep1pt
  ...
  \item Total inter-trip recharge: \textbf{11.62 min / 23.00 kWh}.
  \item On the tapering curve those same 24.47 minutes deliver only $\approx 31.55$ kWh -- a \alert{4.26 kWh shortfall}\ vs.\ the linear plan's 35.81 kWh budget (saved diagnostic).
  \item Solver refuses the deep recharge; the late \texttt{r7} no longer fits. \textbf{Obj 109.07}.
\end{itemize}
\end{block}
\end{columns}

\vspace{6pt}
\begin{block}{The structural takeaway}
The two charging assumptions disagree on whether the third trip is feasible at all. Linear's plan is what an operator would dispatch if charging were really constant-rate. The next slide shows what happens when a real battery has to execute that plan.
\end{block}
\end{frame}

% ================= CROSS EVAL =================
\begin{frame}{Cross-Evaluation --- Linear's Plan Is Infeasible Under Nonlinear}
...
\begin{columns}[T,onlytextwidth]
\column{0.32\textwidth}
\begin{block}{Linear (source)}
\small Obj \textbf{165.51}\\
5 served, 3 trips\\
Recharge: 24.47 min / 35.81 kWh
\end{block}
\column{0.32\textwidth}
\begin{block}{Fixed plan, nonlinear curve}
\small \alert{\textbf{INFEASIBLE}}\\
The 35.81 kWh cannot be delivered inside the inter-trip gap once the slow tail is enforced. Trip 3 starts after \texttt{r7}'s pickup window.
\end{block}
\column{0.32\textwidth}
\begin{block}{Free nonlinear re-optimization}
\small Obj \textbf{109.07}\\
4 served, 3 trips\\
Recharge: 11.62 min / 23.00 kWh
\end{block}
\end{columns}

\vspace{6pt}
\begin{block}{The honest headline}
...
\begin{frame}{Multi-Bus Robustness Check}
\textbf{Concern.} Is the headline pathology a single-bus artefact?\\
\textbf{Re-run.} 2 buses, 2 trip slots each, 8 requests, same Euclidean grid, same seed.

\vspace{6pt}
\centering\small
\begin{tabular}{@{}lllcr@{}}
\toprule
\textbf{Scenario} & \textbf{Mode} & \textbf{Status} & \textbf{Obj.} & \textbf{Served} \\
\midrule
Baseline (multi-bus)         & Linear     & OPT & 305.64 & 7 \\
Baseline (multi-bus)         & Nonlinear  & OPT & 305.64 & 7 \\
Partial recharge (multi-bus) & Linear     & OPT & 305.64 & 7 \\
Partial recharge (multi-bus) & Nonlinear  & OPT & 305.64 & 7 \\
\rowcolor{PurdueGold!25}
Deep recharge (multi-bus)    & Linear     & OPT & \textbf{300.22} & 7 \\
\rowcolor{PurdueGold!25}
Deep recharge (multi-bus)    & Nonlinear  & OPT & \textbf{293.98} & 7 \\
\bottomrule
\end{tabular}
...
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}
\begin{block}{Same pathology, smaller gap}
\small
The deep-recharge linear plan sends bus~2 on a 18.18 kWh inter-trip recharge to fit a three-request second trip. \alert{\textbf{Cross-evaluation again returns INFEASIBLE.}} Free nonlinear re-optimization spreads load across the two buses for 6.24 less profit.
\end{block}
\column{0.49\textwidth}
...
\column{0.43\textwidth}
\centering
\includegraphics[width=\linewidth]{deep_instance_map.png}
\\[2pt]
...
The linear charging assumption is safe when capacity is comfortable and \textbf{misleading when capacity is tight enough to force a deep recharge into the slow tail of the taper}. In that regime it produces a plan that the real battery cannot execute.
\end{block}
...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{Single-bus, $|Q|=7$, $|R|=3$, deep recharge.} Linear obj 165.51 (5 served) vs nonlinear 109.07 (4 served): \alert{\textbf{gap 56.44, and linear's plan is INFEASIBLE under nonlinear}}.
  \item \textbf{Multi-bus, $|K|=2$, $|R|=2$, $|Q|=8$, deep recharge.} Same pathology, smaller magnitude: linear 300.22 vs nonlinear 293.98, \alert{\textbf{linear's plan still INFEASIBLE under nonlinear}}.
  \item \textbf{Three buses, same instance.} The asymmetry collapses --- capacity is too loose for the deep-recharge regime to bind.
  \item Baseline charges \emph{longer} under nonlinear (41.27 vs 24.57 min for 35.95 kWh) because the demand reaches the slow tail of the taper. Tapering charging is not uniformly faster.
\end{itemize}
\end{frame}
...
\textbf{Limitations}
\begin{itemize}\itemsep2pt
  \item PWL approximation, not the explicit bilinear form (intentional MIP-vs-MIQCP trade).
  \item Synthetic instances; no calibration against a published transit benchmark.
...
\begin{itemize}\itemsep2pt
  ...
  \item Larger fleets with column generation on trip patterns.
